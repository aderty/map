...
(function (exports) {
    ...
    // List
    //
    function getListController(model, filter) {
        return function (req, res) {
            function list(objFilter) {
                model.find(objFilter, function (err, result) {
                    if (!err) {
                        res.send(result);
                    } else {
                        res.send(errMsg(err));
            ...
            if (filter) return filter(req, res, list);
            list({});
        }
    }
    ...
    exports.initRoutesForModel = function (options) {
        ...
        path = options.path || options.prefix + '/' + model.modelName.toLowerCase();
        pathWithId = path + '/:id';
        app.get(path, getListController(model, options.filter));
